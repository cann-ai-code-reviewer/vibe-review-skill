...
## 5.5 输出自检清单（必须执行，不合格则修正后再输出）

无论变更大小、是否为代码文件、是否有实质发现，输出格式一律不变。逐项检查：

1. 发现标题是否为 `### #N [等级] 描述` 格式？（禁止 `### 发现 N:` 等变体）
2. 等级标签是否为 `[严重]`/`[一般]`/`[建议]` 之一？
3. 置信度是否为"确定"/"较确定"/"待确认"之一？（禁止"高"/"中"/"低"）
4. 规则字段是否引用了具体规范编号（如2.18.1、红线1.5）？无对应编号时写"无"，禁止自由描述（如"类型安全"）
5. "## 审查发现"下方是否有 `共发现N个问题（严重x / 一般y / 建议z）` 汇总行？
6. 行号是否经过验证？（见下方行号自检）
