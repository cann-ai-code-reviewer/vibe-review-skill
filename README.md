...
反证过程:
  1. 枚举使 not-P 成立的充分条件集合 {C1, C2, ..., Cn}
  2. 对每个 Ci, 用工具验证其真值
  3. 存在 Ci = true  ->  not-P 成立  ->  抑制发现
     所有 Ci = false ->  P 的可信度提升  ->  报告并附证据
     Ci 无法判定    ->  保持不确定性  ->  标"待确认"
```
...
```
必检规则命中 (Section 3)
  -> 工具验证 (Section 2, 升级为"较确定"或保持"待确认")
    -> 约束反证 (Section 4.5, 抑制误报或进一步升级)
      -> 输出
```
